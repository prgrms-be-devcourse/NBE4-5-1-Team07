...
export default function OrderDetailPage({params}: { params: { orderId: string } }) {
    ...
    useEffect(() => {
        const fetchOrderDetails = async () => {
            try {
                // API 호출
                const res = await fetch(`http://localhost:8080/api/v1/non-user/orders/${params.orderId}`, {
                    method: "GET",
                    credentials: "include",
                    headers: {
    ...
    }, [params.orderId]); // 의존성 배열 추가
    // 주문 취소 핸들러
    const handleCancelOrder = async () => {
        try {
            const cancelRes = await fetch(`http://localhost:8080/api/v1/non-user/orders/${params.orderId}/cancel`, {
                method: "PUT",
                credentials: "include",
                headers: {
                    "Content-Type": "application/json",
