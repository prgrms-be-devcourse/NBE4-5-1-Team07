    ...
    const fetchOrderDetails = async () => {
      try {
        ...
        const res = await fetch(
          `http://localhost:8080/api/v1/non-user/orders/${orderId}`,
          {
            method: "GET",
            credentials: "include",
            headers: {
  ...
  const handleCancelOrder = async () => {
    try {
      const cancelRes = await fetch(
        `http://localhost:8080/api/v1/non-user/orders/${orderId}/cancel`,
        {
          method: "PUT",
          credentials: "include",
          headers: {
                ...
                {order.orderItems.map((item, index) => (
                  ...
                  >
                    ...
                    <td className="px-6 py-4 whitespace-nowrap text-sm font-bold text-gray-900">
                      {(item.orderPrice * item.count).toLocaleString("ko-KR")}{" "}
                      원
                    </td>
                  </tr>
                ))}
